...
void Map::allocate_bounding_box_digital() {
  ...
  if (series * parallel_own < children.size()) {
    ...
    bounding_box.height =
        children[0]->bounding_box.height * children.size() / series;
  } else {
    bounding_box.width =
        children[0]->bounding_box.width * children.size() / parallel_own;
  ...
  for (int i = 1; i < children.size(); ++i) {
    if (children[i]->bounding_box.xbar != bounding_box.xbar) {
      // TO DO: merge xbars
      if (children[i - 1]->bounding_box.x +
              children[i - 1]->bounding_box.width +
              children[i]->bounding_box.width <
          bounding_box.width) {
        ...
        children[i]->bounding_box.y =
            children[i - 1]->bounding_box.y + children[i]->bounding_box.height;
      }
  ...
  */
  ...
  if (children[0]->st == PRIMITIVE) {
    bounding_box = Box(new Xbar(), -1, -1, children[0]->bounding_box.width * 3,
                       0, children[0]->bounding_box.height * 3);
  } else {
    bounding_box = Box(new Xbar(), -1, -1,
                       children[0]->bounding_box.width *children.size() / 170,
                       0, children[0]->bounding_box.height * 170);
  }
}
    ...
    if (child->st != PRIMITIVE) {  // 只检查skeleton+primitive的组合
      ...
      if (analog_box->width > XBAR_LENGTH) {  // 宽度过大，按列拆分即可
        while (analog_box->width > XBAR_LENGTH) {
          analog_boxes.push_back(new Box(NULL, -1, -1, XBAR_LENGTH, XBAR_LENGTH,
                                         analog_box->height));
          analog_box->width -= XBAR_LENGTH;
        }
        analog_boxes.push_back(new Box(NULL, -1, -1, analog_box->width,
                                       analog_box->width, analog_box->height));
      }
      if (analog_box->height >
          XBAR_LENGTH) {  // 高度过大，按行拆分，最后额外增加一个阵列累加部分和，这里未考虑需要加法树的情况
        int adder_num = 1;
