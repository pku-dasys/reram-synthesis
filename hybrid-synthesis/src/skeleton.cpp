...
Data *Skeleton::operator[](int index) { return outputs[index]; }
...
void Skeleton::calc_pos() {
  ...
  for (int i = children.size() - 1; ~i; i--) {
    ...
    if (skel->bounding_box.height > XBAR_LENGTH ||
        skel->bounding_box.width > XBAR_LENGTH) {
      skel->calc_pos();
      for (int k = skel->pos.size() - 1; ~k; k--) pos.push_back(skel->pos[k]);
    } else
...
void Skeleton::map_to_crossbar(int obj) {
  for (int i = 0; i < NOCW; i++)
    for (int j = 0; j < NOCH; j++) noc[i][j] = Noc(XBAR_LENGTH * XBAR_LENGTH);
  switch (obj) {
    case 1:
...
double calc_min_utils() {
  ...
  for (int i = 0; i < NOCW; i++)
    for (int j = 0; j < NOCH; j++) utils = min(utils, noc[i][j].utils);
  return utils;
}
...
void Skeleton::map_to_crossbar_throughput() {
  ...
  while (t > 1e-5) {
    ...
    while (true) {
      nocx = rand() % NOCW;
      nocy = rand() % NOCH;
      if (noc[nocx][nocy].skeleton.size() == 0) continue;
...
double calc_area() {
  ...
  for (int i = 0; i < NOCW; i++)
    for (int j = 0; j < NOCH; j++)
      if (noc[i][j].skeleton.size() > 0) area++;
  return area;
}
